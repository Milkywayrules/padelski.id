...
- `api` and `web` use `expose` only (no host publish in git compose). Coolify still injects shared `127.0.0.1:3000/3001` host binds from domain UI config into the deploy artifact — `scripts/coolify/up.sh` writes `docker-compose.override.yml` with `ports: !reset []` per api/web service (including `api-pr-N`) before `docker compose up` auto-merges it. The same override patches missing Traefik `loadbalancer.server.port` labels on PR previews (Coolify #6832).
