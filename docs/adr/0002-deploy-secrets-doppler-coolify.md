...
| **Smoke** | Chained at end of `scripts/coolify/up.sh` (health, config, web root). Auto-detects **loopback** (local host) vs **compose-exec** (Coolify helper — discovers `api`/`api-pr-N` service names). On PR previews, also curls public FQDNs from `SERVICE_URL_*` or Traefik `Host()` rules in merged compose config (`SMOKE_PUBLIC_URLS=auto\|always\|never`). Override with `SMOKE_MODE=loopback\|compose-exec`. |
