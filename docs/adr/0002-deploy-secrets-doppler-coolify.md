...
- `api` and `web` use `expose` only (no host port publish) — Coolify proxy routes by domain on the compose network so main + multiple PR previews can run concurrently without `3000`/`3001` collisions.
