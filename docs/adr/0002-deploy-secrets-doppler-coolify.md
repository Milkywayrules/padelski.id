...
| Variable | Role |
| --- | --- |
| `SERVICE_URL_API` | Public API URL for this deployment (preview subdomain from Preview URL Template) |
| `SERVICE_URL_WEB` | Public web URL for this deployment |
...
The two variable families are generated by **different mechanisms**, which decides how each one has to be set up:

- `SERVICE_URL_<SERVICE>` / `SERVICE_FQDN_<SERVICE>` are derived from the **domain assigned in the Coolify UI**, injected stack-wide for every service that has a domain. There is **no port suffix** — the port belongs in the domain field (`https://host:3001`) and is not part of the public origin, which is `https://host` on 443. The port-suffixed authoring form (`SERVICE_URL_API_3001`) belongs to Coolify *Service* stacks, where Coolify generates the domain and reads the proxy port off the suffix. Referencing the suffixed name from a Docker Compose **application** interpolates to empty.
- `SERVICE_PASSWORD_<ID>` is generated only when Coolify **parses a compose file that references it**. Until the referencing compose is the one stored on the application, the variable does not exist and interpolates to empty.

Every reference uses the `${VAR:?}` required form so a missing value fails `docker compose up` by name instead of starting a broken container.
...
At deploy time compose wires these from `SERVICE_URL_API` and `SERVICE_URL_WEB`.
...
- **`DOPPLER_TOKEN`** must be set in Coolify as **runtime-only** (and a separate preview-scoped token under Preview Deployment Environment Variables). Runtime-only keeps it out of the image build history — see Risks.
...
### Resolved (2026-08-01) — `SERVICE_URL_*` naming and cross-service reuse

Confirmed on a live PR-19 preview deployment. Coolify injects `SERVICE_URL_API` and `SERVICE_URL_WEB` stack-wide, so referencing the web URL from the api service is sound — no per-service assignment needed. The **port suffix was wrong**, however: `SERVICE_URL_API_3001` interpolated to empty, which silently emptied `NEXT_PUBLIC_API_URL`, `BETTER_AUTH_URL`, and `API_CORS_ORIGIN`. Fixed to the un-suffixed names.

The same deployment failed with `postgres is unhealthy` because `SERVICE_PASSWORD_POSTGRES` did not exist — the compose Coolify had parsed still used a plain `${POSTGRES_PASSWORD}` and referenced no magic variables at all, so Coolify had never generated one. On a preview the postgres volume is fresh, and `postgres:16-alpine` refuses to initialise without a non-empty `POSTGRES_PASSWORD`.

### Unverified — `DOPPLER_TOKEN` in image build history

Coolify declares every environment variable as a build `ARG` and passes it via `--build-arg`. A local reproduction confirmed that a `--build-arg` value is recorded in `docker history` output (it is absent from `docker inspect`). So a `DOPPLER_TOKEN` marked as a build variable is readable from the api image's layer history by anyone with Docker access on the host. **Not yet confirmed** whether Coolify passed it as a build arg on the PR-19 build specifically. Mark `DOPPLER_TOKEN` **runtime-only** in Coolify's environment variable settings, which is also Coolify's own recommendation in its build warnings.

Images are built on the VPS and never pushed to a registry, so the exposure is bounded by host access.
