...
- `api` and `web` use `expose` only plus `ports: !reset []` — Coolify otherwise injects shared `127.0.0.1:3000/3001` host binds from domain UI config, which breaks concurrent PR previews; proxy routes by domain on the compose network instead.
