...
| Piece | Behavior |
| --- | --- |
| **Coolify** | v4.1.2 · Docker Compose build pack · custom **build** via `bash scripts/coolify/build.sh --pull` (api then web sequentially — avoids parallel Next.js OOM on small VPS) · custom **start** via `bash scripts/coolify/up.sh -d` |
...
| **Smoke** | Chained at end of `scripts/coolify/up.sh` (health, config, web root). Auto-detects **loopback** (local host) vs **compose-exec** (Coolify helper — discovers `api`/`api-pr-N` service names; published ports are on Docker host, not helper loopback). When `SERVICE_URL_*` is set (previews), also curls public FQDNs (`SMOKE_PUBLIC_URLS=auto\|always\|never`). Override with `SMOKE_MODE=loopback\|compose-exec`. |
...
- `api` and `web` use `expose` only (no host publish in git compose). Coolify still injects shared `127.0.0.1:3000/3001` host binds from domain UI config into the deploy artifact — `scripts/coolify/up.sh` writes `docker-compose.override.yml` with `ports: !reset []` per api/web service (including `api-pr-N`) before `docker compose up` auto-merges it. The same override patches missing Traefik `loadbalancer.server.port` and `traefik.docker.network` labels on PR previews (Coolify #6832), reading router ids from merged `docker compose config` output rather than repo YAML alone.
